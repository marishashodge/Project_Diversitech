...
import server
from model import connect_to_db, db, Company, Category, Review, example_data
from helper import *
...
class FlaskTests(TestCase):
    ...
    #     """Tests to see if the company-not-found page loads."""
    #
    #     result = self.client.get('/search')
    #
    #     self.assertEqual(result.status_code, 200)
    #     self.assertIn('text/html', result.headers['Content-Type'])
    #     self.assertIn('{{% block content %}', result.data)
    #
    # def test_load_company(self):
    #     """Tests to see if the company page loads."""
    #
    #     result = self.client.get('/company/<int:company_id>')
    #
    #     self.assertEqual(result.status_code, 200)
    #     self.assertIn('text/html', result.headers['Content-Type'])
    #     self.assertIn('Diversity Data', result.data)
    #
    # def test_load_review(self):
    #     """Tests to see if the company page loads."""
    #
    #     result = self.client.get('/review/<int:company_id>')
    #
    #     self.assertEqual(result.status_code, 200)
    #     self.assertIn('text/html', result.headers['Content-Type'])
    ...
    #     self.assertNotIn('Gender:', result.data)
    ...
    #
    #     company1 = Company.query.filter(Company.name == "Company1").first()
    #     self.assertEqual(company1.name, "Company1")
    #
